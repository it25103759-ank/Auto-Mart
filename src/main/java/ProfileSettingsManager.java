...
/**
 * ============================================================
 *  ProfileSettingsManager.java
 *  Component 05 — Profile Settings
 *  Student : Siriwardana A.K.L.B.  |  IT25103761
 * ============================================================
 *
 *  This class handles ALL CRUD operations for Profile Settings:
 *
 ...
 *  Composition    : Uses Profile objects (has-a relationship).
...
public class ProfileSettingsManager {
    ...
    /**
     * Creates a ProfileSettingsManager.
     *
     ...
     * a new Profile record to profiles.txt.
     *
     * Validation rules:
     *  - Profile ID must be unique (no duplicates in profiles.txt)
    ...
    public void createProfile() {
        ...
        // ── Collect input ──────────────────────────────────────────────
        System.out.print("  Enter Profile ID (e.g. P001) : ");
        String profileId = scanner.nextLine().trim();

        // Check for duplicate profile ID
        if (profileExists(profileId)) {
            printError("Profile ID '" + profileId + "' already exists. Choose a different ID.");
            return;
        }
     ...
     * READ operation.
     *
     * Prompts for a Profile ID, searches profiles.txt line-by-line, and
     * prints the matching profile using Profile.toString().
     */
    ...
    public void updateUsername() {
        ...
        Profile profile = findProfileById(profileId);
        if (profile == null) {
            printError("Profile ID '" + profileId + "' not found.");
            return;
        }
     ...
     * DELETE operation.
     *
     * Asks for a Profile ID and, after confirmation, removes that record
     ...
     *
     * @return List of all valid Profile objects
     */
    ...
    /**
     * Finds a Profile by its profile ID (case-insensitive).
     *
     ...
     *
     * @param updated The modified Profile object
    ...
    private void ensureFileExists() {
        ...
        try {
            ...
            System.err.println("[ProfileSettingsManager] Could not initialize profiles.txt: " + e.getMessage());
        }
    }
